import enum
import uuid
from sqlalchemy import BigInteger, Column, ForeignKey, String, DateTime, Enum, Table
...
class Textos(database.Base):
    __tablename__ = "TEXTOS"
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    saludo = Column(String)
    cierre = Column(String)
    clienteid = mapped_column(ForeignKey("USUARIOS.id", ondelete="CASCADE"))
    usuario = relationship("Usuarios", back_populates="textos")
...
class Usuarios(database.Base):
    ...
    textos = relationship("Textos", back_populates="usuario")
